...
void addBook() {
    ...
    int newId;
    printf(YELLOW "Enter Book ID   : " RESET);
    scanf("%d", &newId);                                 // Read the new book's ID first for duplicate check
    clearBuffer();                                       // Clear newline left in buffer after scanf

    // Check if a book with the same ID already exists
    for (int i = 0; i < count; i++) {
        if (b[i].id == newId) {                          // Duplicate ID found — reject immediately
            printf(RED "\n  ✘  A book with ID %d already exists!\n" RESET, newId);
            printLine();
            return;                                      // Exit without adding the book
        }
    }
    b[count].id = newId;                                 // ID is unique, store it in current slot

    printf(YELLOW "Enter Book Name : " RESET);
    ...
    printf(YELLOW "Enter Author    : " RESET);
    fgets(b[count].author, 50, stdin);                   // Read author name from keyboard
    b[count].author[strcspn(b[count].author, "\n")] = 0; // Strip trailing newline

    printf(YELLOW "Enter Topic     : " RESET);
    fgets(b[count].topic, 50, stdin);                    // Read topic from keyboard
    b[count].topic[strcspn(b[count].topic, "\n")] = 0;  // Strip trailing newline

    // Check if a book with the exact same name AND topic already exists
    for (int i = 0; i < count; i++) {
        if (strcmp(b[i].name, b[count].name) == 0 &&    // strcmp returns 0 if strings match
            strcmp(b[i].topic, b[count].topic) == 0) {  // Both name and topic must match to be a duplicate
            printf(RED "\n  ✘  A book with the same name and topic already exists!\n" RESET);
            printLine();
...
void displayBooks() {
    ...
    for (int i = 0; i < count; i++) { // Loop from index 0 to count-1 (every stored book)
        ...
        printf(MAGENTA "  %-5d" RESET " %-25s %-20s %-15s %s%-10s%s\n",
               b[i].id, b[i].name, b[i].author, b[i].topic, statusColor, statusText, RESET);
        // Prints all fields in one line: ID (magenta), name, author, topic, colored status
    }
...
int main() {
    ...
    do { // do-while loop: runs at least once, keeps running until user chooses 8 (Exit)
        ...
        printf(WHITE "  1." RESET "  ➕  Add Book\n"        // Option 1
               WHITE "  2." RESET "  📋  Display Books\n"   // Option 2
               WHITE "  3." RESET "  🔍  Search by ID\n"    // Option 3
               WHITE "  4." RESET "  🔎  Search by Topic\n" // Option 4
               WHITE "  5." RESET "  📤  Issue Book\n"      // Option 5
               WHITE "  6." RESET "  📥  Return Book\n"     // Option 6
               WHITE "  7." RESET "  🗑️   Delete Book\n"     // Option 7
               WHITE "  8." RESET "  🚪  Exit\n");           // Option 8
        // Adjacent string literals are joined by the compiler into one printf call
        printLine();
